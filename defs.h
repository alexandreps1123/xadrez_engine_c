#ifndef DEFS_H
#define DEFS_H

#include "stdlib.h"

#define DEBUG

#ifndef DEBUG
#define ASSERT(n)
#else
#define ASSERT(n) \
if(!(n)) { \
printf("%s - Failed", #n); \
printf("On %s ", __DATE__); \
printf("At %s ", __TIME__); \
printf("In File %s ", __FILE__); \
printf("At Line %d\n", __LINE__); \
exit(1); }
#endif

typedef unsigned long long U64;

#define NAME "Parkus 1.0"

// numero de quadrados do tabuleiro
#define BRD_SQ_NUM 120

#define MAXGAMEMOVES 2048

// definir todas as pecas
/* enum de todas as pecas
 * w - indica brancas "white"
 * b - indica negras "black"
 * P - peao
 * N - cavalo
 * B - bispo
 * R - torre
 * Q - dama
 * K - rei
*/
enum { EMPTY, wP, wN, wB, wR, wQ, wK, bP, bN, bB, bR, bQ, bK };

// FILE representa as colunas do tabuleiro a, b, c, e, f, g, h
enum { FILE_A, FILE_B, FILE_C, FILE_D, FILE_E, FILE_F, FILE_G, FILE_H, FILE_NONE };

// RANK representa as linhas do tabuleiro 1, 2, 3, 4, 5, 6, 7, 8
enum { RANK_1, RANK_2, RANK_3, RANK_4, RANK_5, RANK_6, RANK_7, RANK_8, RANK_NONE };

// as cores a serem utilizadas
enum { WHITE, BLACK, BOTH};

// MAPA DO TABULEIRO
// quadrados do tabuleiro
// NO_SQ - 'not sqare' caso algum coordenada esteja fora do tabuleiro
enum {
    A1 = 21, B1, C1, D1, E1, F1, G1, H1,
    A2 = 31, B2, C2, D2, E2, F2, G2, H2,
    A3 = 41, B3, C3, D3, E3, F3, G3, H3,
    A4 = 51, B4, C4, D4, E4, F4, G4, H4,
    A5 = 61, B5, C5, D5, E5, F5, G5, H5,
    A6 = 71, B6, C6, D6, E6, F6, G6, H6,
    A7 = 81, B7, C7, D7, E7, F7, G7, H7,
    A8 = 91, B8, C8, D8, E8, F8, G8, H8, NO_SQ
    
};

// false = 0; true = 1
enum { FALSE, TRUE};

// K = KING, Q = QUEEN
// WKCA = WHITE KING CASTLE (ROQUE)
// 1000 0100 0010 0001
// Permissao para roque
enum { WKCA = 1, WQCA = 2, BKCA = 4, BQCA = 8 };

typedef struct {
	
	int move;
	int castlePerm;
	int enPassant;
	int fiftyMove;
	U64 posKey;
	
} S_UNDO;

// estrutura do tabuleiro
typedef struct {

    int pieces[BRD_SQ_NUM];
    U64 pawns[3];

    int KingSq[2];

    int side;
    int enPassant;
    int fiftyMove;

    int ply;
    int hisPly;
    
    int castlePerm;

    // chave unica de certa posicao
    U64 posKey;

    int pceNum[13];

    // qualquer peca que n seja peao
    int bigPce[3];

    // torres e rainhas
    int majPce[3];

    // bispos e cavalos
    int minPce[3];
    
    S_UNDO history[MAXGAMEMOVES];

    // lista de pecas
    // 13 pecas com possibilidade de existir 10 torres, por exemplo
    int pList[13][10];

} S_BOARD;

/* MACROS */
// paramentros file e rank
// e retorna o equivalente no tabuleiro de 120
#define FR2SQ(f, r) ( (21 + (f) ) + ( (r) * 10 ) )

/* GLOBALS */
extern int Sq120ToSq64[BRD_SQ_NUM];
extern int Sq64ToSq120[64];

/* FUNCTIONS */
extern void AllInit();

#endif
